...
class MCTSPlayer(Player):
    ...
    def make_move(self, game: AlignQuattroGame) -> int:
        ...
        # Priority 1: take an immediate win if available
    ...
    def _would_win(self, game: AlignQuattroGame, col: int, as_red: bool) -> bool:
        ...
        Temporarily places a piece, checks for a win using the existing check_direction_win
        method, then restores the board. No deep copy is made — this is a zero-copy operation
        making it extremely fast to call in tight loops.
    ...
    def _heuristic_move(self, game: AlignQuattroGame) -> int:
        ...
        copying the entire game state, making this extremely fast.
        ...
        # Centre-biased fallback — no copies needed
    ...
    def _simulate(self, game: AlignQuattroGame, root_is_red: bool) -> float:
        ...
        At each step, instead of a purely random move, applies a simple priority:
            1. If the current player can win immediately, take it
            2. If the opponent can win immediately, block it
            3. Otherwise prefer centre columns

        game is mutated directly — callers must pass a copy. _run_one_iteration
        already passes iter_game which is a deep copy, so no additional copy is needed.
        ...
        """
        while game.get_outcome() == "in progress":
            chosen = self._heuristic_move(game)
    ...
    def _reward_from_simulation(self, outcome: str, root_is_red: bool) -> float:
        """Convert a terminal game outcome from a rollout into a reward for backpropagation.
