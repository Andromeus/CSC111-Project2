...
class AlignQuattroVisualization:
    ...
    def run_game_loop(self) -> None:
        ...
        while self.running:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if GAME_STATES[self.game_state] == "menu":
                        if RECTS["lower middle"].collidepoint(event.pos):
                            ...
                            player_1_choice += 1
                            self.change_player_types(
                                True, player_1_choice, red_dif, red_dag, red_heuristic)
                        elif RECTS["right"].collidepoint(event.pos):
                            player_2_choice += 1
                            self.change_player_types(
                                False, player_2_choice, yel_dif, yel_dag, yel_heuristic)
                        elif RECTS["left difficulty"].collidepoint(event.pos) and player_1_choice % 3 == 2:
                            red_dif += 1
                            red_dif %= 4
                            self.change_player_types(
                                True, player_1_choice, red_dif, red_dag, red_heuristic)
                            self.type_customization_text(
                                True, red_dif, red_dag, red_heuristic)
                            ...
                            self.change_player_types(
                                False, player_2_choice, yel_dif, yel_dag, yel_heuristic)
                            self.type_customization_text(
                                False, yel_dif, yel_dag, yel_heuristic)
            ...
            if GAME_STATES[self.game_state] == "menu":
                upper_header = self.get_font(1).render(
                    "Welcome to ALIGNQUATTRO", True, COLOR_DICTIONARY["red"])
    ...
    def draw_menu(self, c1: int = 1, c2: int = 0, red_dif: int = 0, yel_dif: int = 0, red_dag: bool = True,
        ...
        middle_header = self.get_font(0).render(
            "START", True, COLOR_DICTIONARY["yellow"], COLOR_DICTIONARY["red"])
        middle_header_rect = middle_header.get_rect()
        middle_header_rect.center = (1280 // 2, 440)

        lower_header = self.get_font(0).render(
            "DATA", True, COLOR_DICTIONARY["yellow"], COLOR_DICTIONARY["red"])
        lower_header_rect = lower_header.get_rect()
        lower_header_rect.center = (1280 // 2, 540)

        player1_choice_header = self.get_font(0).render(
            "Red Player", True, COLOR_DICTIONARY["red"])
        player1_choice_rect = player1_choice_header.get_rect()
        player1_choice_rect.center = (1280 // 2 - 280, 380)

        player2_choice_header = self.get_font(0).render(
            "Yellow Player", True, COLOR_DICTIONARY["red"])
    ...
    def change_player_types(self, is_left: bool, choice: int, difficulty: int, is_dag: bool, use_h: bool) -> None:
        ...
        choice_header = self.get_font(0).render(
            message, True, COLOR_DICTIONARY["yellow"])
        choice_rect = choice_header.get_rect()
        choice_rect.center = (xpos, 440)

        pygame.draw.rect(
            self.screen, COLOR_DICTIONARY['red'], RECTS[side], 0, 10, 10, 10, 10)
        ...
        if choice % num_options == 0:
            ...
            adjusted_player = player_mcts.MCTSPlayer(
                difficulty_dictionary[difficulty], math.sqrt(2), is_dag, use_h)
            custom_color = COLOR_DICTIONARY['red']

        if is_left:
            for left_rect in ["left difficulty", "left tree", "left heuristic"]:
                pygame.draw.rect(self.screen, custom_color,
                                 RECTS[left_rect], 0, 10, 10, 10, 10)
            self.red = adjusted_player
        else:
            for right_rect in ["right difficulty", "right tree", "right heuristic"]:
                pygame.draw.rect(self.screen, custom_color,
                                 RECTS[right_rect], 0, 10, 10, 10, 10)
    ...
    def type_customization_text(self, is_left: bool, diff: int, is_dag: bool, is_h: bool) -> None:
        ...
        if is_left:
            ...
            for left_rect in ["left difficulty", "left tree", "left heuristic"]:
                pygame.draw.rect(
                    self.screen, COLOR_DICTIONARY['red'], RECTS[left_rect], 0, 10, 10, 10, 10)
        else:
            x_pos = 1150
            for right_rect in ["right difficulty", "right tree", "right heuristic"]:
                pygame.draw.rect(
                    self.screen, COLOR_DICTIONARY['red'], RECTS[right_rect], 0, 10, 10, 10, 10)
        ...
        diff_text = self.get_font(0).render(
            diff_dict[diff], True, COLOR_DICTIONARY["white"])
        diff_rect = diff_text.get_rect()
        diff_rect.center = (x_pos, 440)

        dag_text = self.get_font(0).render(
            dag_dict[is_dag], True, COLOR_DICTIONARY["white"])
        dag_rect = dag_text.get_rect()
        dag_rect.center = (x_pos, 540)

        h_text = self.get_font(0).render(
            heuristic_dict[is_h], True, COLOR_DICTIONARY["white"])
    ...
    def show_data(self) -> None:
        ...
        header = title_font.render(
            "Data Visualizations", True, COLOR_DICTIONARY["blue"])
        text = font.render("Click anywhere to return to menu.",
                           True, COLOR_DICTIONARY["blue"])
        ...
        vs_baseline = pygame.image.load(
            'images/experiments_against_baseline.jpg')
        ...
        heuristics_vs_not = pygame.image.load(
            'images/Heuristics_vs_Non_Heuristics_Win_Rate.jpg')
        heuristics_vs_not = pygame.transform.scale(
            heuristics_vs_not, (600, 350))
